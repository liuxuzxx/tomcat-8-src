...
public class Http11NioProcessor extends AbstractHttp11Processor<NioChannel> {
	...
	public Http11NioProcessor(int maxHttpHeaderSize, NioEndpoint endpoint, int maxTrailerSize,
		...
		System.out.println(this.getClass().getName() + "这个不是启动的时候就载入的");
	}
	...
	protected boolean handleIncompleteRequestLineRead() {
		...
		if (((InternalNioInputBuffer) inputBuffer).getParsingRequestLinePhase() < 2) {
	...
	protected boolean breakKeepAliveLoop(SocketWrapper<NioChannel> socketWrapper) {
		...
		if (sendfileData != null && !getErrorState().isError()) {
			...
			if (socketWrapper.getSocket().getPoller().processSendfile(key, (KeyAttachment) socketWrapper, true)) {
		...
		case REQ_HOST_ATTRIBUTE: {
			if (socketWrapper == null) {
				...
				if (socketWrapper.getRemoteHost() == null) {
					...
					if (socketWrapper.getRemoteHost() == null) {
						if (socketWrapper.getRemoteAddr() == null && inetAddr != null) {
							socketWrapper.setRemoteAddr(inetAddr.getHostAddress());
						}
						if (socketWrapper.getRemoteAddr() != null) {
	...
	protected boolean prepareSendfile(OutputFilter[] outputFilters) {
		...
		if (fileName != null) {
			...
			sendfileData.pos = ((Long) request.getAttribute(org.apache.coyote.Constants.SENDFILE_FILE_START_ATTR))
					.longValue();
			sendfileData.length = ((Long) request.getAttribute(org.apache.coyote.Constants.SENDFILE_FILE_END_ATTR))
