 ...
 */
/**
 * jsp的解析出现问题，我就首先从这个jsp的解析下手，反正研究整个tomcat的时间很长
 * 我们就用这个契机进行一个jsp的解析的研究，我们不需要研究的很深，但是一定要搞定这个问题
 * 目前，我认识到的问题就是：tomcat启动的时候并不会去读取这个jsp文件，不会去编译这个文件，不然，肯定是启动的报错的干活
 * 所以，应该是第一次访问的时候，开始编译这个jsp文件
 * 
 * @author liuxu
 *
 */
class Validator {
	...
	private static class DirectiveVisitor extends Node.Visitor {
		...
		/**
		 * 其实看这个牛逼的人写代码有的时候，还是需要很多的硬编码，但是这个硬编码到底是正确的还是错误的
		 * 我觉得关键是要看是否是合适的，如果是合适的就是正确的
		 */
	...
	private static class ValidateVisitor extends Node.Visitor {
		 ...
		 */
		/**
		 * jsp的解析出现异常的时候就是从这个方法首先抛出来的，根据java的异常抛出机制， 首先抛出来的异常就是这个第一个出现异常的地方
		 * 
		 * @param compiler
		 */
