...
public class Http11Nio2Protocol extends AbstractHttp11JsseProtocol<Nio2Channel> {
	...
	public Http11Nio2Protocol() {
		...
		System.out.println(this.getClass().getName() + "Http开始初始化这个对象接受信息了");
	}
	...
	protected static class Http11ConnectionHandler extends AbstractConnectionHandler<Nio2Channel, Http11Nio2Processor>
			implements Handler {
		...
		public void release(SocketWrapper<Nio2Channel> socket) {
			Processor<Nio2Channel> processor = connections.remove(socket.getSocket());
			if (processor != null) {
		 ...
		 */
		@Override
		public void release(SocketWrapper<Nio2Channel> socket, Processor<Nio2Channel> processor,
		...
		public Http11Nio2Processor createProcessor() {
			Http11Nio2Processor processor = new Http11Nio2Processor(proto.getMaxHttpHeaderSize(),
					(Nio2Endpoint) proto.endpoint, proto.getMaxTrailerSize(), proto.getAllowedTrailerHeadersAsSet(),
		...
		@Override
		protected Processor<Nio2Channel> createUpgradeProcessor(SocketWrapper<Nio2Channel> socket,
				ByteBuffer leftoverInput, UpgradeToken upgradeToken) throws IOException {
			return new Nio2Processor(proto.endpoint, socket, leftoverInput, upgradeToken,
